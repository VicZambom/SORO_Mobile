...
import { ArrowLeft, MapPin, Clock, ImageIcon, Users, Edit2 } from 'lucide-react-native';
      ...
      <View style={tw`bg-white rounded-xl p-5 shadow-sm mb-20 border border-gray-100`}>
        ...
        {steps.map((step, i) => (
          <View key={i} style={tw`flex-row mb-6`}>
             ...
             <View style={[tw`w-5 h-5 rounded-full items-center justify-center z-10`, step.active ? tw`bg-orange-500` : tw`bg-gray-300`]} />
...
// 2. ABA MÍDIA
const MidiaTab = ({ ocorrencia, onMediaUploaded }: { ocorrencia: OcorrenciaFull; onMediaUploaded: (novaMidia: Midia) => void }) => {
  ...
  const handleUpload = async (asset: ImagePicker.ImagePickerAsset) => {
    setUploading(true);
    try {
      // Prepara o FormData para envio de arquivo
      ...
      const response = await api.post(
        `/api/v2/ocorrencias/${ocorrencia.id_ocorrencia}/midia`, 
        formData, 
        {
      ...
      onMediaUploaded(response.data.data); // Atualiza a lista localmente

    } catch (error) {
      console.error('Erro no upload:', error);
      Alert.alert('Erro', 'Falha ao enviar a mídia. Tente novamente.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <ScrollView style={tw`flex-1 bg-gray-50 p-5`}>
      ...
      <View style={tw`flex-row flex-wrap justify-between`}>
        ...
        {/* Botão Adicionar */}
        <TouchableOpacity 
          style={tw`w-[31%] aspect-square bg-white border border-slate-300 rounded-xl items-center justify-center mb-3 border-dashed`}
          onPress={pickImage}
          disabled={uploading}
        >
          {uploading ? (
...
export const DetalheAndamentoScreen = () => {
  ...
  // Callback para atualizar o estado local quando uma mídia é enviada
  const handleMediaUploaded = (novaMidia: Midia) => {
    setOcorrencia((prev) => {
      if (!prev) return null;
      return {
        ...prev,
      ...
      <View style={tw`flex-1 bg-white rounded-t-3xl overflow-hidden shadow-lg`}>
        ...
        >
            ...
            <Tab.Screen name="Mídia" options={{ title: `Mídia (${ocorrencia.midias.length})` }}>
                {() => <MidiaTab ocorrencia={ocorrencia} onMediaUploaded={handleMediaUploaded} />}
            </Tab.Screen>
